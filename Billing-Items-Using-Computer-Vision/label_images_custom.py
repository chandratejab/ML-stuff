...
# def predictor(image,labels,graph):
...
#   response = run_graph(image_data, labels, FLAGS.input_layer, FLAGS.output_layer,FLAGS.num_top_predictions)

#   return response

def predictor(images,labelsPath,graph):
  """Runs inference on an image."""
  # if argv[1:]:
  #   raise ValueError('Unused Command Line Args: %s' % argv[1:])
  # import pdb;pdb.set_trace()
  ...
  for image in images:
    ...
    if not tf.gfile.Exists(labelsPath):
      tf.logging.fatal('labels file does not exist %s', labels)
